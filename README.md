...
- 支持全押、边池（主池 + 多个边池）、短码全押不构成完整加注；平局均分，余下的零头按
  「按钮左侧最近的赢家」依次发放。
- 每次行动 5 分钟，超时自动过牌或弃牌；开手前有 5 秒的公平贡献窗口。
- 每手结束弹出结算窗，逐座位列出本手净输赢、亮出的牌与牌型、剩余筹码：所有真人点「确认，继续」
  后立刻开下一手，有人没点则由 10 分钟兜底窗口自动继续，不会把牌桌卡住。
- 亮牌口径：赢家总是亮，弃牌者不亮。摊牌时每位未弃牌者亮出最佳五张与牌型（高牌、一对……
  同花顺），弃牌收池时只亮赢家的底牌。
- 牌桌中央有行动播报横幅：全押最大（大字＋抖动＋品牌绿）、加注中等、弃牌安静淡出；发公共牌、
  每手开始/结算、比赛结束与牌局暂停各有对应播报。两端同一套语气与时长。牌桌新增独立语音/音效开关，支持跟注、加注、过牌、弃牌、全押中文短语音与发牌/筹码/结算反馈；默认关闭。
- 新手教程是盖在当前页面上的弹窗，六步讲完「认识牌桌 → 一手牌怎样进行 → 轮到你时怎么选 →
  用最好的五张牌比大小（含牌面示例）→ 练习一次跟注（答对才放行）→ 看懂结算」，首次进大厅
  自动弹一次，各页顶栏都能随时重看；完整规则与常见问题在规则页。
- 配色为浅灰底 + 低饱和绿牌桌 + 单一品牌绿强调色（无金色/铜色点缀），浏览器端
  `apps/web/static/styles.css` 与小程序 `apps/wechat/app.wxss` 共用同一套变量，
  `apps/wechat/test/static.test.ts` 会逐值比对两端。
...
结算窗里的亮牌（`rooms/showdown.ts`）也走同一条口径：只有该亮的座位才带 `cards`，
弃牌者的底牌不会进任何响应，历史弃牌仍然只在整场结束后的核验页公开。
...
| `POKER_SETTLE_MS` / `POKER_BOT_THINK_MS` | `600000` / `2500` | **仅开发模式**：结算兜底窗口与机器人思考时长（毫秒）。生产模式一律忽略并打印提示，避免把对局压到玩家来不及反应 |
...
见 [本地多人联机测试指南](docs/local-multiplayer.md)：不同浏览器模拟真人、同一 Wi-Fi 手机互联、网页与小程序同桌，以及部署前仍需完善的登录环节。
