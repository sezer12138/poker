...
describe('倒计时与超时', () => {
  ...
  it('行动窗口恰好 5 分钟，超时自动弃牌并写入事件', async () => {
    const {roomId, players} = await humanTable();
    ...
    assert.equal((await server.view(players[0]!, roomId)).actionTimeoutMs, 300000, '视图要告诉客户端行动时限');

    await server.clock.advance(299999);
    await drain();
    const waiting = (await server.view(players[0]!, roomId));
    assert.equal(waiting.hand.street, 'preflop', '第 299999 毫秒仍应等玩家行动');
  ...
  it('没人点确认时，结算 10 分钟后自动开始下一手，手号递增且重新开启贡献窗口', async () => {
    const {roomId, players} = await humanTable();
    await contributeAll(server, roomId, players);
    const actor = await waitingActor(server, roomId, players);
    ...
    assert.equal(settled.nextHandAt! - settled.serverTime, 600000);
