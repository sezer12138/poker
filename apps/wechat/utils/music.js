/**
 * 背景音乐：用 wx.createWebAudioContext 实时合成，不加载任何音频文件
 * （小程序主包体积与版权都因此没有负担，与 web 端 apps/web/static/js/music.js 同一套和弦）。
 *
 * 约束：模块顶层不访问 wx —— 基础库没有该能力时（2.19.0 以下、或开发者工具未模拟）
 * 直接判断为不支持，牌桌把开关置灰，而不是让玩家点一个永远不响的按钮。
 * 注意：本模块的调度逻辑只在 Node 里用假上下文验证过，真机发声未经验证。
 */
...
/** 小程序里真正建上下文的办法。放函数里而不是模块顶层，装载本模块不会碰 wx。 */
function defaultAudioContext() {
  ...
  try {
    ...
    return null; // 能力存在但建不起来（无音频设备等）时静默降级为没有音乐。
  }
}

/**
 * 合成背景音乐。返回的每个方法都可以安全地重复调用：没有 Web Audio 的环境里
 * start 是空操作，state().supported 为 false，牌桌据此把开关置灰而不是假装在放。
 */
function createMusic(options) {
  const config = options || {};
  const createContext = config.createContext || defaultAudioContext;
  ...
  function playNote(frequency, at) {
    ...
    // 不写成链式 connect：小程序端的 connect 不保证返回目标节点。
  ...
  return {
    ...
    state: function () {
      ...
      return { playing: playing, muted: muted, supported: supported };
    }
  };
}
